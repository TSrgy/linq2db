			...
			bool     skipUndefined     = false)
		{
			...
			if (!skipNull && !skipPass && nullSql != null)
			{
				...
				if (!skipDefinedNull && dataType != DataType.Undefined)
				{
					...
					var parameters = Enumerable.Range(0, paramCount).Select((_, i) => new DataParameter($"p{i}", value, dataType)).ToArray();
				...
				if (!skipUndefinedNull)
				{
					...
					var parameters = Enumerable.Range(0, paramCount).Select((_, i) => new DataParameter($"p{i}", value)).ToArray();
					id = conn.Execute<int?>(sql, parameters);
					Assert.That(id, Is.EqualTo(1));
				}
			}
